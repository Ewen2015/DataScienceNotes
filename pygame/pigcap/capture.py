...
class Pig:
    ...
    def __init__(self, position, num, field_size, field):
        '''
        初始化方法，定义基本属性
        position：列表，初始坐标
        num：整数，每头猪的编号
...
def main():
    ...
    #目标提示线的颜色
    target_line_color=pygame.Color(yellow) #(153,153,0)
    #人类矩形和猪类矩形的颜色，和他们的尺寸（像素）
    human_color=pygame.Color(blue) #(0,191,255)
    pig_color=pygame.Color(red) #(255,20,147)
    size=5
    ...
    for i in range(100):
        for j in range(50):
            #相邻两头猪间隔为一个size
            pig_list.append(Pig([30+2*j,60+2*i], i*10+j+1, (field_width, field_height), field))

    for i in range(20):
        for j in range(50):
            #随机为每个人分配目标
            human_list.append(Human([20+2*j,10+2*i], pig_list[random.randint(0,len(pig_list)-1)], pig_list, (field_width, field_height), field))
    ...
    while True:
        ...
        #检查猪抓完没有，并绘制猪数量的提示文字
        if not len(pig_list):
            screen.blit(pygame.font.Font(font,50).render("mission completed in around %d hours"%(frame*4//60), 1, black),(200,400))
            ...
            screen.blit(pygame.font.Font(font,24).render("pigs left: {}".format(Pig.count), 1, yellow),(500,2))
        ...
        # #底部签名及信息
        # screen.blit(pygame.font.Font(font,22).render("幽蓝伊梦",1,(92,255,255)),(1050,750))
        # screen.blit(pygame.font.Font(font,22).render("点个赞吧",1,(92,255,255)),(280,750))
        # screen.blit(pygame.font.Font(font,22).render("投个币吧(*^▽^*)",1,(92,255,255)),(430,750))
